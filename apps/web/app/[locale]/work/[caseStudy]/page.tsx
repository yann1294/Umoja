...
import { getPublishedCmsCaseStudy } from "@/lib/cms/public-rendering";
...
export function generateStaticParams() {
  return CASE_STUDY_SLUGS.map((caseStudy) => ({ caseStudy }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale, caseStudy } = await params;
  if (!hasLocale(routing.locales, locale)) notFound();
  const study = (await getPublishedCmsCaseStudy(locale, caseStudy)) ?? getCaseStudy(caseStudy);
  if (!study) notFound();
...
export default async function CaseStudyPage({ params }: Props) {
  ...
  const title = localize(study.title, locale);
  const items = [
    [t("challenge"), study.challenge],
    [t("contribution"), study.contribution],
    [t("result"), study.result],
    [t("status"), study.status],
    [t("lessons"), study.lessons],
  ] as const;
  return (
    <>
      <Breadcrumbs
        ...
        items={[
          ...
          { label: t("workTitle"), href: `/${locale}/work` },
          { label: title },
        ]}
      />
      <ContentHero
        ...
        illustrativeLabel={study.illustrative ? localize(study.illustrativeLabel, locale) : undefined}
      />
