...
| Audit-insertion rollback | NOT RUN | No safe, fixture-scoped transaction fault-injection capability is exposed through the remote Supabase project; no shared audit bypass, public failure endpoint, or persistent trigger change was introduced. | Requires temporary database-owner capability to install/remove a transaction-scoped audit-insert fault for child mutation, profile/private-details save, and moderation/feedback cases, with before/after row and audit snapshots |
| True concurrency/conflict handling | NOT RUN | No overlapping two-client evidence was executed in this run; sequential stale requests are not represented as proof. Existing RPCs use row locks/expected-state checks, but this acceptance item remains unverified. | Requires a controlled overlapping remote test for same-version saves, competing moderation decisions, and applicant edit versus approval, followed by row/audit consistency checks |
| Responsive/Axe/real 200% zoom | NOT RUN | Existing Gate A evidence covers prior routes only; no new profile/skills/portfolio/availability/moderation matrix or genuine Chrome-for-Testing zoom screenshots were captured in this run. | New-route inspection at required widths/orientations, long-content and state fixtures, Axe, focus/touch/overflow checks, and real 200% browser zoom |
