...
The reviewed owner-run method is a single PostgreSQL transaction over synthetic UUIDs only. It creates a
temporary trigger function in `pg_temp` and a transaction-scoped `BEFORE INSERT` trigger on
`public.audit_logs`; the trigger raises only when `NEW.target_id` equals the current synthetic fixture UUID.
...
profile/child/private-details, feedback, and audit counts before the outer transaction is rolled back. A
statement timeout and client disconnect are safeguards for a stuck trigger. The owner must verify that the
temporary trigger/function no longer exists, then run exact-prefix Auth/table cleanup. Creating the temporary
function/trigger and connecting with a privileged PostgreSQL role require database-owner or equivalent DDL
privileges; the Supabase service key cannot execute PostgreSQL DDL and is intentionally insufficient.
...
| Audit-insertion rollback | NOT RUN | Exact reviewed method documented below. Existing remote access is REST/service-key access only; it cannot safely create a PostgreSQL trigger/function or run transaction-scoped DDL. No shared audit bypass, public failure endpoint, or persistent trigger change was introduced. | Owner must run the reviewed local PostgreSQL script with a short-lived privileged connection: `BEGIN`; create a `pg_temp` audit-failure trigger on `public.audit_logs` matching only the synthetic target UUID; run each case; snapshot business/feedback/audit rows; `ROLLBACK` (or timeout and disconnect); verify the trigger/function is absent and fixtures are cleaned. The connection string must come from the owner’s local password manager/env, never chat/browser/commit. |
| True concurrency/conflict handling | FAIL | Temporary authenticated overlap harness `/private/tmp/umoja-concurrency-small.mjs` issued genuinely simultaneous same-version RPC saves; one request did not return a controlled conflict within 60s and was aborted. Exact-prefix cleanup removed all 3 users. The additive migration `20260830210000_profile_concurrency_guards.sql` is applied remotely, but this result means same-version concurrency is not yet accepted; moderation and edit-versus-approval overlaps were not run after the timeout. | Diagnose the remote lock/timeout with database observability or owner-run SQL, then rerun all three overlap cases. Required outcomes remain one success/one controlled conflict, consistent profile/private-details/feedback/audit state, and no stale approval of changed content |
| Responsive/Axe/real 200% zoom | FAIL / NOT RUN | `tests/e2e/design-system.spec.ts` and `tests/e2e/workspace-auth.spec.ts` ran across all configured widths/orientations: structural responsive, keyboard focus, reduced motion, overflow and touch checks passed for the corrected fixture; 7 reviewed screenshot comparisons differ because the fixture now avoids denied anonymous RLS reads, and committed profile/skills/portfolio/availability/moderation routes were not covered. Chrome-for-Testing artifacts are in `/private/tmp/umoja-chrome-zoom-qa/`; both keyboard and OS-level attempts measured unchanged CSS viewport `1024x790` before/after, so 200% is not accepted. The run found zero page overflow and zero sub-44px controls, with clipped descendants on several complex routes. | Individually review/update only the 7 changed shell baselines, add isolated synthetic fixtures for the five Prompt 12 screens/states in EN/FR, resolve clipped descendants, and repeat with a browser method that demonstrably changes CSS viewport to approximately half width |
