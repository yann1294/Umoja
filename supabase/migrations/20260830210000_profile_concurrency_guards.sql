-- Serialize profile writes and bind moderation to the version that was reviewed.
...
begin
  ...
  if expected_updated_at is not null and current_updated_at is distinct from expected_updated_at then raise exception 'stale profile' using errcode = '40001'; end if;
  ...
  on conflict (user_id) do update set professional_name=excluded.professional_name, locale=excluded.locale, country_code=excluded.country_code, public_bio=excluded.public_bio, public_slug=excluded.public_slug, visibility=excluded.visibility, public_consent_at=case when consent_given then coalesce(public.profiles.public_consent_at, now()) else null end, consent_version=case when consent_given then coalesce(public.profiles.consent_version, 'profile-public-v1') else null end, publication_state=excluded.publication_state, updated_at=now()
...
drop function if exists public.moderate_profile(uuid, public.profile_publication_state, public.profile_publication_state, text);
create function public.moderate_profile(
  profile_user_id uuid, decision public.profile_publication_state,
  expected_state public.profile_publication_state, feedback text,
...
begin
  ...
  if expected_updated_at is not null and result.updated_at is distinct from expected_updated_at then raise exception 'stale moderation decision' using errcode = '40001'; end if;
...
revoke all on function public.moderate_profile(uuid, public.profile_publication_state, public.profile_publication_state, text, timestamptz) from public;
grant execute on function public.moderate_profile(uuid, public.profile_publication_state, public.profile_publication_state, text, timestamptz) to authenticated;
