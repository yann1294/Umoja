-- Stale profile versions are application conflicts, not retryable serialization failures.
-- PT409 is the PostgREST-supported SQLSTATE for an HTTP 409 response.
...
begin
  ...
  if not found then raise exception 'stale moderation decision' using errcode = 'PT409'; end if;
  if expected_updated_at is not null and result.updated_at is distinct from expected_updated_at then raise exception 'stale moderation decision' using errcode = 'PT409'; end if;
